...
import { createDataColumn } from './translate_widget_data_options';
import { IncludeMembersFilter, Panel, PanelItem, WidgetType } from './types';
...
const getDrilldownSelections = (
  ...
  if (item?.parent && item.through && 'filter' in item.through.jaql) {
    ...
    const dateFormat = 'level' in jaql && jaql.level && mask && mask[jaql.level];
