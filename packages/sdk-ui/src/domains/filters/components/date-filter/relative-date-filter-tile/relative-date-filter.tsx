...
import { Popover } from '@/shared/components/popover.js';
      ...
      <div className="csdk-flex csdk-justify-between csdk-items-center csdk-h-6 csdk-my-1">
            ...
            elements={Object.keys(DATE_OPS_MAP).map((op) => {
              return (
                <div
                  onClick={() => {
                    if (operator !== op) {
                      onAnyValueChange(op, undefined, undefined, undefined, filter.config);
                    }
                  }}
                >
                  {t(DATE_OPS_MAP[op])}
        ...
        <BasicInput
          ...
          callback={(newCount: string) => {
            if (newCount !== '') {
              onAnyValueChange(undefined, Number(newCount), undefined, undefined, filter.config);
            }
          }}
          required={true}
          disabled={disabled}
                ...
                <div
                  onClick={() => {
                    if (levelAttr.granularity !== level) {
                      onAnyValueChange(undefined, undefined, level, undefined, filter.config);
                    }
                  }}
                >
          ...
          <CalendarDateSelector
            ...
            onDateChanged={(selectedDate: dayjs.Dayjs) => {
              if (selectedDate !== anchor) {
                onAnyValueChange(undefined, undefined, undefined, selectedDate, filter.config);
