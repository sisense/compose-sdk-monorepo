...
import { getJtdClickHandler } from './jtd/jtd-handlers.js';
import { JtdConfig, JtdTarget, JumpToDashboardConfig } from './jtd/jtd-types.js';
import { useJtdInternal } from './use-jtd.js';
...
// Mock dependencies
vi.mock('@/infra/contexts/modal-provider/use-modal', () => ({
  useModal: vi.fn(),
}));

vi.mock('@/domains/widgets/components/widget-by-id/utils', () => ({
...
vi.mock('@/shared/utils/combine-handlers', () => ({
...
describe('useJtdInternal', () => {
  ...
  beforeEach(async () => {
    ...
    // Setup mocks
    const { useModal } = await import('@/infra/contexts/modal-provider/use-modal');
    const { isChartWidgetProps, registerDataPointContextMenuHandler } = await import(
      '@/domains/widgets/components/widget-by-id/utils'
    );
  ...
  describe('getJtdClickHandler', () => {
    ...
    it('should handle widgets without formula context gracefully', async () => {
      const { isChartWidgetProps } = await import(
        '@/domains/widgets/components/widget-by-id/utils'
      );
      vi.mocked(isChartWidgetProps).mockReturnValue(true);
