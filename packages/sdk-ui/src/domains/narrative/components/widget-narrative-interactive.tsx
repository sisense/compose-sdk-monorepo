...
export const NarrativeTopSlotRow = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
  gap: 10px;
...
export const IconDiv = styled.div<Themable>`
...
/** Sparkle AI icon rendered next to every narrative. */
export function NarrativeAiIcon({ theme }: Themable) {
...
function ThumbsUpGlyph({ color, bold = false }: { color: string; bold?: boolean }) {
  const boldProps = bold
    ? ({ stroke: color, strokeWidth: 1, paintOrder: 'stroke fill' } as const)
    : {};
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
...
function ThumbsDownGlyph({ color, bold = false }: { color: string; bold?: boolean }) {
  const boldProps = bold
    ? ({ stroke: color, strokeWidth: 1, paintOrder: 'stroke fill' } as const)
    : {};
  return (
...
function NarrativeFeedbackRow({
  ...
  const [selection, setSelection] = useState<FeedbackSelection>(null);
  ...
  const onClick = useCallback(
    (type: 'up' | 'down') => {
      const next: FeedbackSelection = selection === type ? null : type;
      setSelection(next);
      if (next !== null) {
        onSend(next === 'up' ? 1 : -1);
      } else {
  ...
  const buttonSx = (type: 'up' | 'down') => ({
    padding: '2px',
    backgroundColor: selection === type ? hoverBg : 'transparent',
    ...
    <div className="csdk-flex csdk-items-center">
      <Tooltip title={t('ai.buttons.correctResponse')}>
        <IconButton
          ...
          sx={buttonSx('up')}
        >
          <ThumbsUpGlyph color={iconColor} bold={selection === 'up'} />
        </IconButton>
      </Tooltip>
      <Tooltip title={t('ai.buttons.incorrectResponse')}>
        <IconButton
          ...
          sx={buttonSx('down')}
        >
          <ThumbsDownGlyph color={iconColor} bold={selection === 'down'} />
        </IconButton>
      </Tooltip>
    </div>
  );
    ...
    <FeedbackColumn ref={ref} theme={themeSettings}>
      {renderContent(
        <NarrativeFeedbackRow
          ...
          onSend={sendFeedback}
          onCancelSend={() => sendFeedback.cancel()}
