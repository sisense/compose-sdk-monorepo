...
describe('translateQueryFromJSON', () => {
  ...
  describe('trend and forecast', () => {
    it('should expand styled measure with trend only to base + trend measure', () => {
      const mockQueryJSON = {
        dimensions: ['DM.Category.Category'],
        measures: [
          {
      ...
      expect(query.measures[0].name).toBe('Total Revenue');
      expect(query.measures[1].composeCode).toContain('measureFactory.trend');
      expect(query.measures[1].name).toBe('$trend_Total Revenue');
    });

    it('should expand styled measure with forecast only to base + forecast measure', () => {
      const mockQueryJSON = {
        dimensions: ['DM.Category.Category'],
        measures: [
          {
      ...
      expect(query.measures[1].composeCode).toContain('measureFactory.forecast');
      expect(query.measures[1].name).toBe('$forecast_Total Cost');
    });

    it('should expand styled measure with both trend and forecast to three measures', () => {
      const mockQueryJSON = {
        dimensions: ['DM.Category.Category'],
        measures: [
          {
      ...
      expect(result).toHaveProperty('data');
      const query = (result as { success: true; data: any }).data;
      expect(query.measures).toHaveLength(3);
      expect(query.measures[0].composeCode).toBe(
        "measureFactory.sum(DM.Commerce.Revenue, 'Total Revenue')",
      );
      expect(query.measures[1].composeCode).toContain('measureFactory.trend');
      expect(query.measures[1].name).toBe('$trend_Total Revenue');
      expect(query.measures[2].composeCode).toContain('measureFactory.forecast');
      expect(query.measures[2].name).toBe('$forecast_Total Revenue');
    });

    it('should not expand when measure is already measureFactory.trend (single measure)', () => {
      const mockQueryJSON = {
        dimensions: ['DM.Category.Category'],
        measures: [
          {
      ...
      expect(query.measures[0].composeCode).toContain('measureFactory.trend');
      expect(query.measures[0].name).toBe('Trend Revenue');
    });

    it('should apply sortType to base and still add trend measure', () => {
      const mockQueryJSON = {
        dimensions: ['DM.Category.Category'],
        measures: [
          {
      ...
      expect(query.measures[0].getSort()).toBe(Sort.Descending);
      expect(query.measures[1].composeCode).toContain('measureFactory.trend');
      expect(query.measures[1].name).toBe('$trend_Total Revenue');
    });
  });
