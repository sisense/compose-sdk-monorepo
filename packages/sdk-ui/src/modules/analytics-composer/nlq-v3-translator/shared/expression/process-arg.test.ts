...
vi.mock('../utils/attribute-helpers.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../utils/attribute-helpers.js')>();
  return {
    ...actual,
    isDateLevelAttribute: vi.fn(
      (attribute: unknown) => (attribute as { type?: string })?.type === 'datelevel',
    ),
    getAttributeTypeDisplayString: vi.fn((attribute: unknown) => {
      const type = (attribute as { type?: string })?.type;
      if (type === 'text-attribute') return 'text';
      if (type === 'numeric-attribute') return 'numeric';
      if (type === 'datelevel') return 'date/datetime';
      return 'unknown';
    }),
  };
});
