...
export function processNode(input: NodeInput): QueryElement {
  ...
  if (!schema) {
    const knownFunctions = Object.keys(FACTORY_FUNCTION_SCHEMAS);
    const fnMatch = findBestMatch(functionPath, knownFunctions, (fn) => fn);
    const suggestion =
      fnMatch && fnMatch.distance <= SUGGESTION_THRESHOLD ? ` Did you mean '${fnMatch.best}'?` : '';
    throw new Error(
      `${actualPathPrefix}function: Unknown function '${functionPath}'.${suggestion}`,
    );
  }
