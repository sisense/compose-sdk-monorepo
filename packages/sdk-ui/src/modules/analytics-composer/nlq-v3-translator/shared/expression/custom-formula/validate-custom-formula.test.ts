...
describe('formula-validation', () => {
  describe('validateFormulaReferences', () => {
    ...
    describe('operator syntax validation', () => {
      ...
      it('should pass for CASE WHEN expressions (shared formula / Sisense syntax)', () => {
        const formula =
          'COUNT([FFC04-BC7]) / COUNT(CASE WHEN [1FF18-6D2] = 1 THEN [416DE-C53] END)';
        const context = {
          'FFC04-BC7': 'DM.Posts.acceptedanswerid',
          '1FF18-6D2': 'DM.Posts.posttypeid',
          '416DE-C53': 'DM.Posts.id',
        };
        const result = validateFormulaReferences(formula, context);

        expect(result.isValid).toBe(true);
        expect(result.errors.filter((e) => e.includes('Invalid syntax'))).toHaveLength(0);
        expect(result.errors.filter((e) => e.includes('WHEN'))).toHaveLength(0);
      });

      it('should pass for COUNT(CASE WHEN ... THEN 1 END) style formulas', () => {
        const formula =
          'COUNT(CASE WHEN [03784-688] = 2 THEN 1 END) * COUNT(CASE WHEN [03784-688] = 3 THEN 1 END) / POWER(SUM([009E4-F42]), 2)';
        const context = {
          '03784-688': 'DM.Votes.votetypeid',
          '009E4-F42': 'DM.Posts.score',
        };
        const result = validateFormulaReferences(formula, context);
      ...
      it('should pass for CASE WHEN with AND in condition', () => {
        const formula = 'COUNT(CASE WHEN [107A4-FD1] = 0 AND [BA6B5-77B] = 1 THEN 1 END)';
        const context = {
          '107A4-FD1': 'DM.Posts.answercount',
          'BA6B5-77B': 'DM.Posts.posttypeid',
        };
        const result = validateFormulaReferences(formula, context);
      ...
      it('should pass for CASE WHEN with OR in condition (regression: OR allowlisted)', () => {
        const formula = 'COUNT(CASE WHEN [A] = 1 OR [B] = 2 THEN 1 END)';
        const context = {
          A: 'DM.Some.AttrA',
          B: 'DM.Some.AttrB',
        };
      ...
      it('should pass for simple CASE [expr] WHEN value THEN result (regression: CASE before bracket)', () => {
        const formula = 'SUM(CASE [X] WHEN 1 THEN 10 WHEN 2 THEN 20 ELSE 0 END)';
        const context = { X: 'DM.Some.Code' };
        const result = validateFormulaReferences(formula, context);
  ...
  describe('edge cases', () => {
    ...
    describe('parseTimeDiffCalls', () => {
      ...
      it('should extract multiple time-diff calls (case-insensitive)', () => {
        ...
        expect(result[0].functionName).toBe('DDIFF');
        expect(result[0].ref1).toBe('end');
        expect(result[0].ref2).toBe('start');
        expect(result[1].functionName).toBe('YDIFF');
        expect(result[1].ref1).toBe('a');
        expect(result[1].ref2).toBe('b');
      });
    });
    ...
    describe('time-diff validation', () => {
      ...
      it('should error when time-diff arg points to measure', () => {
        ...
        const context = {
          ...
          start: { function: 'measureFactory.sum', args: ['DM.Commerce.Revenue'] },
        };
        const result = validateFormulaReferences(formula, context, {
          schemaIndex,
          errorOnUnusedContext: false,
        ...
        expect(result.isValid).toBe(false);
        expect(result.errors.some((e) => e.includes('points to a measure'))).toBe(true);
        expect(result.errors.some((e) => e.includes('DDIFF'))).toBe(true);
      });

      it('should error when schemaIndex present and ref is non-datetime column', () => {
        const formula = 'Avg(DDIFF([cat], [date]))';
        const context = {
          cat: 'DM.Commerce.Category',
        ...
        expect(result.isValid).toBe(false);
        expect(
          result.errors.some((e) => e.includes('non-datetime') || e.includes('Category')),
        ).toBe(true);
      });
      ...
      it('should error when date level does not match time-diff function', () => {
        ...
        expect(
          result.errors.some(
            (e) =>
              e.includes('expects date level') &&
              e.includes('Days') &&
              (e.includes('Years') || e.includes("'Years'")),
          ),
        ).toBe(true);
      });

      it('should pass when both refs are datetime with correct level', () => {
        const formula = 'Avg(DDIFF([end], [start]))';
        const context = {
          end: 'DM.Commerce.Date.Days',
          start: 'DM.Commerce.Date.Days',
        };
        const result = validateFormulaReferences(formula, context, {
          schemaIndex,
          errorOnUnusedContext: false,
        ...
        expect(result.isValid).toBe(true);
        expect(result.errors.filter((e) => e.includes('DDIFF'))).toHaveLength(0);
      });

      it('should pass when ref has no level (inference left to translation)', () => {
        const formula = 'Avg(DDIFF([end], [start]))';
        const context = {
          end: 'DM.Commerce.Date',
          start: 'DM.Commerce.Date',
        };
        const result = validateFormulaReferences(formula, context, {
          schemaIndex,
          errorOnUnusedContext: false,
      ...
      it('should push parse error when table not found (schemaIndex present)', () => {
        const formula = 'Avg(DDIFF([a], [b]))';
        const context = {
          a: 'DM.NonExistentTable.Col.Days',
          b: 'DM.Commerce.Date.Days',
        };
        const result = validateFormulaReferences(formula, context, {
          schemaIndex,
