...
export class AbstractDataLoadService implements DataLoadServiceI {
  ...
  constructor(events?: EventEmitter) {
    ...
    this.logger = debug.create('AbstractDataLoadService');
  }
  ...
  clone(): DataLoadServiceI {
    ...
    throw new Error(`"${instanceName}.clone" should be overridden in subclass`);
  }
  ...
  load(jaql: JaqlRequest): Promise<any> {
    if (!jaql) {
      throw new Error(`${this.logger.getName()} "jaql" is not defined for "load" method`);
    }
  ...
  loadFromServer() {
    const instanceName = this.logger.getName();
    throw new Error(`"${instanceName}.loadFromServer" should be overridden in subclass`);
  }
  ...
  cancelQuery(): Promise<void> {
    const instanceName = this.logger.getName();
    const error = new Error(`"${instanceName}.cancelQuery" should be overridden in subclass.`);
  ...
  notifyAboutDataChunk(type: string, chunk?: any, isFromCache = false) {
    this.logger.log('notifyAboutDataChunk', type, chunk, isFromCache);
  ...
  notifyAboutDataLoaded(data?: any) {
    ...
    if (typeof this.loadPromiseResolve === 'function') {
      ...
      this.logger.log('notifyAboutDataLoaded');
    }
  ...
  notifyAboutDataError(data?: any) {
    ...
    if (typeof this.loadPromiseReject === 'function') {
      ...
      this.logger.log('notifyAboutDataError');
    }
    this.loadPromiseReject = undefined;
    this.loadPromiseResolve = undefined;
  }
