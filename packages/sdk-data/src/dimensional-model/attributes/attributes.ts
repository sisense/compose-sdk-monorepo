 ...
 * carries a `formula` and a `context`, but it is consumed as a dimension and produces
 * categorical/grouping values. Its JAQL is emitted with `type: "calculated_dimension"`.
 *
 * Only the `text` data type is supported, so its element `type` is {@link MetadataTypes.CalculatedAttribute}
 * (mirroring how a plain attribute uses {@link MetadataTypes.Attribute}).
 *
 * @internal
 */
export class DimensionalCalculatedAttribute extends DimensionalElement implements Attribute {
  /**
   * @internal
   */
  readonly __serializable: string = 'DimensionalCalculatedAttribute';

  /**
   * Defines the calculated attribute's expression (the formula)
   */
  readonly expression: string;

  /**
   * Defines the calculated attribute's context
   */
   ...
   */
  sort(sort: Sort): Attribute {
    return new DimensionalCalculatedAttribute(
      this.name,
      this.expression,
      this.context,
      this.description,
  ...
  serialize(): JSONObject {
    ...
    result.context = Object.fromEntries(
      Object.getOwnPropertyNames(context).map((key) => [
        key,
        context[key]?.serialize ? context[key].serialize() : context[key],
      ]),
    );
 ...
 * @internal
 */
export const isDimensionalCalculatedAttribute = (
  v: AnyObject,
): v is DimensionalCalculatedAttribute => {
  return Boolean(v && v.__serializable === 'DimensionalCalculatedAttribute');
};
...
/**
 * Creates a {@link DimensionalCalculatedAttribute} (a calculated dimension) from the given JSON object.
 *
 * Accepts both the JAQL form (`{ type: 'calculated_dimension', formula, context }`) and the
 * serialized form (`{ expression, context }`). Context items are themselves parsed
 * into dimensional elements.
 *
 * @param json - JSON object representing the calculated attribute
 * @returns A new calculated attribute instance
 * @internal
 */
export function createCalculatedAttribute(json: any): Attribute {
  ...
  const context: Record<string, any> = {};
  Object.getOwnPropertyNames(json.context || {}).forEach((key) => {
    context[key] = create(json.context[key]);
  });
